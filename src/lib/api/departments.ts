...
import type { ApiErrorBody, Department, DepartmentPayload, SpringPage } from "@/lib/types/hrEntities";
...
function base(): string {
  return `${getApiBaseUrl()}/api/v1/departments`;
}
...
export async function fetchDepartmentPage(
  ...
  if (!res.ok) throw new Error(`Không tải được phòng ban (${res.status})`);
  return res.json();
}
...
export async function updateDepartment(id: number, body: DepartmentPayload): Promise<Department> {
