...
public final class AppointmentDecisionSpecifications {

    private AppointmentDecisionSpecifications() {
    }

    public static Specification<AppointmentDecision> filter(UUID userId, String keyword) {
        return (root, query, cb) -> {
            ...
            if (keyword != null && !keyword.isBlank()) {
                ...
                ors.add(cb.like(root.get("decisionNumber"), pattern, '\\'));
                ors.add(cb.like(root.get("appointedTitle"), pattern, '\\'));
                ors.add(cb.and(
