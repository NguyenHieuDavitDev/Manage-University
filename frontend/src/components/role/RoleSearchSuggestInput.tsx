...
import { fetchRoleSuggestions } from "@/lib/api/roles";
...
type Props = {
  ...
  onPickSuggestion?: (item: RoleSuggestion) => void;
  placeholder?: string;
  name?: string;
  debounceMs?: number;
  minChars?: number;
...
export function RoleSearchSuggestInput({
  value,
  onChange,
  onPickSuggestion,
  ...
  /** Tránh hiện “Không có gợi ý” trước khi request debounce chạy xong */
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void (async () => {
        ...
        setLoading(true);
        try {
          const data = await fetchRoleSuggestions(q, limit);
          if (cancelled) return;
          setItems(data);
  ...
  function pick(item: RoleSuggestion) {
    clearBlurTimer();
    focusRef.current = false;
    closePanel();
  ...
  return (
    <div ref={wrapRef} className="relative min-w-0 flex-1">
      ...
      <input
        ...
        aria-label="Từ khóa tìm kiếm vai trò"
      />
            ...
            items.map((item, idx) => (
              <li key={item.id} role="option" aria-selected={highlight === idx}>
                <button
                  ...
                  onClick={() => pick(item)}
                >
                  <span className="font-mono text-xs text-[#3c8dbc]">{item.roleCode}</span>
