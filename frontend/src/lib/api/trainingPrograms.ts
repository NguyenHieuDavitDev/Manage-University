...
function base(): string {
  return `${getApiBaseUrl()}/api/v1/training-programs`;
}

export async function fetchTrainingProgramPage(
  page: number,
  size = 20,
  sort = "id,desc",
  q?: string | null
): Promise<SpringPage<TrainingProgram>> {
  ...
  if (!res.ok) throw new Error(`Không tải được chương trình đào tạo (${res.status})`);
  return res.json();
}

export async function createTrainingProgram(body: TrainingProgramPayload): Promise<TrainingProgram> {
